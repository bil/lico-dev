...
cdef class DiskSinkDriver(sink_driver.SinkDriver):
  def __cinit__(self):
    ...
    # self.signalsTableArr[0].number = 3
    ...
    # self.signalsTableArr[1].number = 1
    # self.signalsTableArr[1].unit = "TODO"
    # self.signalsTableArr[1].data = NULL

      # raw number sigs
    self.signalsTableArr[1 + self.num_extra_cols].colName = "r_i1_scalar_out"
    if 'int' in 'int8':
      self.signalsTableArr[1 + self.num_extra_cols].SQLtype = "INTEGER"
    else: # float
      self.signalsTableArr[1 + self.num_extra_cols].SQLtype = "REAL"
    ...
    # self.signalsTableArr[2].data = NULL
      # vector sigs
    curr_index = 2 # if suffixes specified
    ...
    # self.signalsTableArr[3].number = 4
    # self.signalsTableArr[3].unit = "TODO"
    # self.signalsTableArr[3].data = NULL
      # msgpack sigs
    self.signalsTableArr[3 + self.num_extra_cols].colName = "m_f8_matrix_out"
    self.signalsTableArr[3 + self.num_extra_cols].SQLtype = "BLOB"
  ...
  cdef bint flushToDisk(self, bint newDB=False) nogil:
    ...
    while (self.pSignalsStructStart != self.pSignalsStructEnd):
      ...
      for i in range(3):
        if "int" in 'int16':
          if "64" in 'int16':
            sql_bind_int64(self.signalsStmt, 1 + self.num_extra_cols, &(self.pSignalsStructStart.vector_ints[i]))
          else:
            sql_bind_int(self.signalsStmt, 1 + self.num_extra_cols, "int16", &(self.pSignalsStructStart.vector_ints[i]))
        else: # float
          sql_bind_double(self.signalsStmt, 1 + self.num_extra_cols, "int16", &(self.pSignalsStructStart.vector_ints[i]))
        self.num_extra_cols += 1
      self.num_extra_cols -= 1
      # raw number signal
      if "int" in "int8":
        if "64" in "int8":
          sql_bind_int64(self.signalsStmt, 2 + self.num_extra_cols, self.pSignalsStructStart.scalar_out)
        else:
          sql_bind_int(self.signalsStmt, 2 + self.num_extra_cols, "int8", self.pSignalsStructStart.scalar_out)
      else: # float
        sql_bind_double(self.signalsStmt, 2 + self.num_extra_cols, "int8", self.pSignalsStructStart.scalar_out)
      # vector signal
      for i in range(4):
        if "int" in 'double':
          if "64" in 'double':
            sql_bind_int64(self.signalsStmt, 3 + self.num_extra_cols, &(self.pSignalsStructStart.vector_floats[i]))
          else:
            sql_bind_int(self.signalsStmt, 3 + self.num_extra_cols, "double", &(self.pSignalsStructStart.vector_floats[i]))
        else: # float
          sql_bind_double(self.signalsStmt, 3 + self.num_extra_cols, "double", &(self.pSignalsStructStart.vector_floats[i]))
        self.num_extra_cols += 1
      self.num_extra_cols -= 1
      # msgpack signal
      ...
      sql_bind_blob(self.signalsStmt, 4 + self.num_extra_cols, self.mpSignalsMatrix_outBuf.data, self.mpSignalsMatrix_outBuf.size, SQLITE_STATIC)
